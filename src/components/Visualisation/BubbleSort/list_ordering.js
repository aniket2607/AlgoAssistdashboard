...
export default function ListOrdering() {
  ...
  const startVisual = () => {
    const timeoutHandler = setInterval(() => setArray(bubbleSort),rows.speed)
    setArray({data:rows['data'],tHandler:timeoutHandler, isComparing: rows['isComparing'], isSwapping:rows['isSwapping'], i: rows['i'], j:rows['j'], last_sorted:rows['last_sorted'],speed:rows.speed, allStates: rows.allStates, type: rows.type})
  }
  ...
  const pause_resume = ()=>{
    if(rows['tHandler'] !== 0){
      ...
      setArray({data:rows['data'],tHandler:0, isComparing: rows['isComparing'], isSwapping:rows['isSwapping'], i: rows['i'], j:rows['j'], last_sorted:rows['last_sorted'],speed:rows.speed,allStates: rows.allStates, type: rows.type})

    }
    else{
      
      const resumeHandler = setInterval(()=> setArray(bubbleSort),rows.speed)
      setArray({data:rows['data'],tHandler:resumeHandler, isComparing: rows['isComparing'], isSwapping:rows['isSwapping'], i: rows['i'], j:rows['j'], last_sorted:rows['last_sorted'],speed:rows.speed, allStates: rows.allStates, type: rows.type})

    }
    return
  }
  const changeSpeed = (e,x) => {
      clearInterval(rows.tHandler)
      setArray({data:rows['data'],tHandler:0, isComparing: rows['isComparing'], isSwapping:rows['isSwapping'], i: rows['i'], j:rows['j'], last_sorted:rows['last_sorted'],speed:x, allStates: rows.allStates, type: rows.type})      
  }
     ...
     const changeType = () => {
      if(rows.type == 1){
        ...
        setArray({data:[...temp],tHandler:0, isComparing: false, isSwapping:false, i: -1, j:-1, last_sorted:9,speed:rows.speed, allStates: [], type: 0})
       }
       else{
         clearInterval(rows.tHandler)
         let temp = [...rows.data]
       ...
       setArray({data:[...temp],tHandler:0, isComparing: false, isSwapping:false, i: -1, j:-1, last_sorted:9,speed:rows.speed, allStates: [], type: 1})
     }
    }
  return (
        ...
        {data && transitions.map(({ item, props: { y, x,...rest }, key }, index) => (
          ...
          style={{ zIndex: data.length - index, transform: y.interpolate(y => `translate3d(${y}px,0,0)`), ...rest }}>
            ...
            <div className="cell" style={{lineHeight:100+2*rows.data[index].value+"px"}} >
                ...
                <div className={`highlight${rows['i']===index || rows['j']===index ? "" : "-hidden"}`} id={index}>
